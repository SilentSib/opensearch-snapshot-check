...
def getSnapshotRepository(os_instance, snapshotRepository):
    try:
        if client.snapshot.SnapshotClient(os_instance).get_repository(
...
def getSnapshots(os_instance, snapshotRepository):
    try:
        return client.snapshot.SnapshotClient(os_instance).get(
            repository=snapshotRepository, snapshot="_all"
        )
    except Exception as e:
        logging.error(f"There was an exception: {e}. Exiting.")
    return False
    ...
    try:
        ...
        if len(snapshotsList["snapshots"]) > 0:
            ...
            date = datetime.strptime(
                snapshotsList["snapshots"][snapshotIndex]["end_time"],
                "%Y-%m-%dT%H:%M:%S.%fZ",
            ).strftime("%m/%d/%y")
...
def main():
    ...
    if (
        not instance
        or not opensearchUsername
        or not opensearchPassword
        or not snapshotRepository
    ):
    ...
    if getSnapshotRepository(os_client, snapshotRepository):
        ...
        if snapshotsCount > 0:
            lastSnapshot = getSnapshotDetails(
                os_client, snapshotRepository, snapshotsCount - 1
            )
            lastSnapshotYear = "20" + lastSnapshot["date"][6:8]
            lastSnapshotMonth = lastSnapshot["date"][0:2]
            lastSnapshotDay = lastSnapshot["date"][3:5]
            lastSnapshotDate = (
                lastSnapshotYear + "-" + lastSnapshotMonth + "-" + lastSnapshotDay
            )
            lastSnapshotState = lastSnapshot["state"]
            if lastSnapshotState == "SUCCESS" and lastSnapshotDate == str(todaysDate):
                print(
                    f"The last snapshot was successfully executed today {todaysDate}."
                )
            else:
                logging.error(
                    "No snapshot has been successfully created today, please manually check the configuration!"
                )
        else:
